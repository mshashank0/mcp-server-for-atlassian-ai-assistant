...
async function runTests() {
  ...
  try {
    ...
    searchResponse.issues.slice(0, 3).forEach((issue: any, idx: number) => {
    ...
    if (histories.length > 0) {
      histories.slice(0, 2).forEach((entry: any, idx: number) => {
    ...
    if (components.length > 0) {
      components.slice(0, 5).forEach((component: any, idx: number) => {
    ...
    if (versions.length > 0) {
      versions.slice(0, 5).forEach((version: any, idx: number) => {
    ...
    try {
      ...
      Object.keys(roles).slice(0, 5).forEach((roleName: string, idx: number) => {
    ...
    try {
      ...
      if (myPermissions.permissions) {
        ...
        grantedPerms.slice(0, 5).forEach((key: string, idx: number) => {
    ...
    issueTypes.forEach((type: any, idx: number) => {
    ...
    try {
      ...
      if (statuses.length > 0) {
        statuses.slice(0, 5).forEach((statusGroup: any, idx: number) => {
    ...
    try {
      ...
      priorities.slice(0, 5).forEach((priority: any, idx: number) => {
    ...
    try {
      ...
      resolutions.slice(0, 5).forEach((resolution: any, idx: number) => {
    ...
    try {
      ...
      if (epicIssues.length > 0) {
        epicIssues.slice(0, 3).forEach((issue: any, idx: number) => {
    ...
    try {
      ...
      if (allSprints.length > 0) {
        allSprints.slice(0, 3).forEach((sprint: any, idx: number) => {
    ...
    boards.slice(0, 5).forEach((board: any, idx: number) => {
    ...
    try {
      ...
      if (allActiveSprints.length > 0) {
        allActiveSprints.slice(0, 3).forEach((sprint: any, idx: number) => {
    ...
    if (worklogs.length > 0) {
      worklogs.slice(0, 3).forEach((worklog: any, idx: number) => {
    ...
    transitions.forEach((transition: any, idx: number) => {
    ...
    fields.slice(0, 10).forEach((field: any, idx: number) => {
    ...
    if (customFields.length > 0) {
      customFields.slice(0, 5).forEach((field: any, idx: number) => {
        console.log(`  ${idx + 1}. ${field.name} (${field.id})`);
      });
    }
    console.log('');
    ...
    searchedFields.forEach((field: any, idx: number) => {
      console.log(`  ${idx + 1}. ${field.name} (${field.id})`);
    });
    console.log('');
    ...
    try {
      ...
      if (fieldKeys.length > 0) {
        fieldKeys.slice(0, 5).forEach((key: string, idx: number) => {
    ...
    if (ENABLE_WRITE_TESTS) {
      ...
      batchIssues.forEach((issue: any, idx: number) => {
      ...
      try {
        ...
        const newVersion = await jira.createProjectVersion(
          TEST_PROJECT_KEY,
          versionName,
          undefined,
          undefined,
