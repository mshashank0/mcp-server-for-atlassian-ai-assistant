    ...
    try {
        switch (name) {
            ...
            case 'confluence_get_page_content': {
                ...
                const result = await confluence.getPageContent(args.page_id, {
                    convertToMarkdown: args.convert_to_markdown,
                    enableHeadingAnchors: args.enable_heading_anchors,
                });
                return {
            ...
            case 'confluence_get_page_ancestors': {
                const result = await confluence.getPageAncestors(args.page_id);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'confluence_get_page_by_title': {
                const result = await confluence.getPageByTitle(args.space_key, args.title, {
                    convertToMarkdown: args.convert_to_markdown,
                    enableHeadingAnchors: args.enable_heading_anchors,
                });
                return {
            ...
            case 'confluence_get_space_pages': {
                const result = await confluence.getSpacePages(args.space_key, args.start, args.limit, {
                    convertToMarkdown: args.convert_to_markdown,
                    enableHeadingAnchors: args.enable_heading_anchors,
                });
                return {
            ...
            case 'confluence_update_page': {
                ...
                const result = await confluence.updatePage({
                    ...
                    title: args.title,
                    body: args.body,
                    updateMode: args.update_mode,
                    versionComment: args.version_comment,
                    isMinorEdit: args.is_minor_edit,
                    isMarkdown: args.is_markdown,
                    enableHeadingAnchors: args.enable_heading_anchors,
                });
                return {
            ...
            case 'confluence_get_page_children': {
                const result = await confluence.getPageChildren(args.page_id, args.start, args.limit, args.expand, {
                    convertToMarkdown: args.convert_to_markdown,
                    enableHeadingAnchors: args.enable_heading_anchors,
                });
                return {
            ...
            case 'confluence_search': {
                const result = await confluence.search(args.cql, {
                    limit: args.limit,
                    start: args.start,
                    spacesFilter: args.spaces_filter,
                });
                return {
                    content: [
                        {
            ...
            case 'confluence_search_user': {
                const result = await confluence.searchUser(args.cql, args.limit);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'confluence_get_page_comments': {
                const result = await confluence.getPageComments(args.page_id, args.return_markdown);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'confluence_add_comment': {
                ...
                const result = await confluence.addComment(args.page_id, args.content, args.is_markdown);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'confluence_get_page_labels': {
                const result = await confluence.getPageLabels(args.page_id);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'confluence_add_page_label': {
                const result = await confluence.addPageLabel(args.page_id, args.label_name);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'confluence_get_spaces': {
                const result = await confluence.getSpaces(args.start, args.limit);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'confluence_get_user_contributed_spaces': {
                const result = await confluence.getUserContributedSpaces(args.limit);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'confluence_get_user_details_by_username': {
                const result = await confluence.getUserDetailsByUsername(args.username, args.expand);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'confluence_get_page_attachments': {
                const result = await confluence.getPageAttachments(args.page_id, args.start, args.limit);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'confluence_attach_file': {
                ...
                const result = await confluence.attachFile(args.page_id, args.file_path, args.comment);
                return {
                    content: [
                        {
                            type: 'text',
