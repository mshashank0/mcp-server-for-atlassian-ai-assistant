...
async function runTests() {
    ...
    try {
        ...
        const textContent1 = pageContent1.find(item => item.type === 'text');
        if (!textContent1) {
        ...
        const textContent = pageContent.find(item => item.type === 'text');
        if (!textContent) {
        ...
        const textMdContent = pageMarkdown.find(item => item.type === 'text');
        if (!textMdContent) {
        ...
        console.log('-'.repeat(60));
        const pageByTitle = await confluence.getPageByTitle(TEST_SPACE_KEY, TEST_PAGE_TITLE);
        if (pageByTitle) {
        ...
        if (ENABLE_WRITE_TESTS) {
            ...
            let attachedFile;
            try {
                attachedFile = await confluence.attachFile(TEST_PAGE_ID, `./${testFileName}`, 'Test attachment from MCP server');
...
// Run tests
runTests();
//# sourceMappingURL=confluence.test.js.map