...
import { getConfluenceConfig } from '../shared/config.js';
...
export class ConfluenceService {
    ...
    constructor() {
        ...
        this.client = new HttpClient(this.config.baseUrl, this.config.apiToken);
    ...
    async getPageContent(pageId, options) {
        ...
        if (options?.convertToMarkdown) {
            ...
            bodyHtml = this.preprocessor.processPageContent(bodyHtml, true, baseUrl, options.enableHeadingAnchors);
        }
        // Create text content
        const textResult = {
            type: 'text',
    ...
    async getPageAncestors(pageId) {
        try {
            ...
            throw new Error(`Failed to get page ancestors: ${error.message}`);
        }
    }
    /**
     ...
     */
    async getPageByTitle(spaceKey, title, options) {
        try {
     ...
     */
    async getSpacePages(spaceKey, start = 0, limit = 25, options) {
        try {
            const url = '/rest/api/content';
            const response = await this.client.get(url, {
                params: {
            ...
            return results.map((page) => {
                ...
                // Process content based on options
                if (options?.convertToMarkdown) {
                    content = this.preprocessor.processPageContent(content, true, baseUrl, options.enableHeadingAnchors);
                    contentFormat = 'markdown';
                }
                const result = {
                    id: page.id,
    ...
    async updatePage(input) {
        try {
            ...
            const pageInfo = await this.client.get(urlGet);
            ...
            if (input.isMarkdown) {
                try {
                    ...
                    throw new Error(`Failed to convert Markdown to HTML: ${conversionError.message}\n` +
            ...
            const response = await this.client.put(urlPut, data);
            return {
                id: response.id,
                title: response.title,
                type: response.type || 'page',
     ...
     */
    validateContentFormat(content, isMarkdown) {
        if (isMarkdown) {
     ...
     */
    async getPageChildren(pageId, start = 0, limit = 25, expand = 'version', options) {
        try {
            ...
            throw new Error(`Failed to get page children: ${error.message}`);
        }
    }
    /**
     * Delete a page
     ...
     */
    async searchUser(query, limit = 10) {
        try {
            // Try approach 1: Direct user search by username
            try {
                const url1 = '/rest/api/user';
                const response1 = await this.client.get(url1, {
            ...
            try {
                ...
                const response2 = await this.client.get(url2, {
     ...
     */
    async getPageComments(pageId, returnMarkdown = true) {
        try {
     ...
     */
    async addComment(pageId, content, isMarkdown = false) {
        try {
            const url = `/rest/api/content`;
            // Process content based on format
    ...
    async getPageLabels(pageId) {
        try {
            ...
            throw new Error(`Failed to get page labels: ${error.message}`);
        }
    }
    /**
    ...
    async addPageLabel(pageId, labelName) {
        try {
            ...
            throw new Error(`Failed to add page label: ${error.message}`);
        }
    }
    // ========================================
     ...
     */
    async getSpaces(start = 0, limit = 25) {
        try {
     ...
     */
    async getUserContributedSpaces(limit = 250) {
        try {
            ...
            throw new Error(`Failed to get user contributed spaces: ${error.message}`);
        }
    }
    // ========================================
     ...
     */
    async getUserDetailsByUsername(username, expand) {
        try {
    ...
    async getCurrentUserInfo() {
        try {
            ...
            throw new Error(`Failed to get current user info: ${error.message}`);
        }
    }
    // ========================================
     ...
     */
    async getPageAttachments(pageId, start = 0, limit = 25) {
        try {
     ...
     */
    async attachFile(pageId, filePath, comment) {
        try {
            ...
            throw new Error(`Failed to attach file: ${errorMessage}${errorDetails ? `\nDetails: ${errorDetails}` : ''}`);
        }
    }
    /**
    ...
    // ========================================
    async getPageImages(pageId, bodyHtml) {
        try {
            const url = `/rest/api/content/${pageId}/child/attachment`;
            const response = await this.client.get(url, {
                params: { limit: 10 },
                ...
                if (mediaType.startsWith('image/')) {
                    ...
                    if (fileName && bodyHtml.includes(fileName)) {
                        ...
                        try {
                            ...
                            console.warn(`Failed to fetch image ${fileName}: ${imageError.message}`);
                        }
                    }
                }
            }
            ...
            console.warn(`Failed to get page attachments for image extraction: ${error.message}`);
            return [];
        }
    }
}
