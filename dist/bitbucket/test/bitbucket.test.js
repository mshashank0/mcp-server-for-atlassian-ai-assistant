...
async function runTests() {
    ...
    try {
        ...
        console.log('-'.repeat(60));
        const commitsResponse = await bitbucket.getCommits(TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_BRANCH_NAME, 0, 5);
        ...
        // Run PR-specific tests with TEST_PR_ID
        {
            ...
            console.log('-'.repeat(60));
            const prResponse = await bitbucket.getPullRequest(TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID);
            ...
            console.log('-'.repeat(60));
            const prCommitsResponse = await bitbucket.getPullRequestCommits(TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID, 0, 5);
            ...
            console.log('-'.repeat(60));
            const activityResponse = await bitbucket.getPullRequestActivity(TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID, 0, 5);
            ...
            console.log('-'.repeat(60));
            const commentsResponse = await bitbucket.getPullRequestComments(TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID, 0, 5);
            ...
            console.log('-'.repeat(60));
            const tasksResponse = await bitbucket.getPullRequestTasks(TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID);
            ...
            console.log('-'.repeat(60));
            const diff = await bitbucket.getPullRequestDiff(TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID);
            ...
            console.log('-'.repeat(60));
            const patch = await bitbucket.getPullRequestPatch(TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID);
        ...
        console.log('-'.repeat(60));
        const fileResponse = await bitbucket.getFileContent(TEST_PROJECT_KEY, TEST_REPO_SLUG, 'README.md');
        ...
        console.log('-'.repeat(60));
        const structureResponse = await bitbucket.getRepoStructure(TEST_PROJECT_KEY, TEST_REPO_SLUG, '', 2);
        ...
        console.log('-'.repeat(60));
        const branchModelResponse = await bitbucket.getRepositoryBranchingModel(TEST_PROJECT_KEY, TEST_REPO_SLUG);
        ...
        const reviewersResponsePersonal = await bitbucket.getEffectiveDefaultReviewers(TEST_PROJECT_KEY, TEST_REPO_SLUG);
        if ('error' in reviewersResponsePersonal) {
            ...
            if (reviewersResponsePersonal.conditions && reviewersResponsePersonal.conditions.length > 0) {
                reviewersResponsePersonal.conditions.slice(0, 3).forEach((condition, idx) => {
        ...
        if (ENABLE_WRITE_TESTS) {
            ...
            */
            ...
            addCommentResponse = await bitbucket.addPullRequestComment(TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID, testComment);
            ...
            createTaskResponse = await bitbucket.createPullRequestTask(TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID, testTaskText);
            ...
            console.log('-'.repeat(60));
            const approveResponse = await bitbucket.approvePullRequest(TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID);
            ...
            console.log('-'.repeat(60));
            const prDetailsForUpdate = await bitbucket.getPullRequest(TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID);
            if ('error' in prDetailsForUpdate) {
                console.log('⚠ Could not get PR details for update test');
            }
            else {
                const updatePRResponse = await bitbucket.updatePullRequest(TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID, prDetailsForUpdate.version, prDetailsForUpdate.title, `${prDetailsForUpdate.description || ''}\n\n_Updated by test at ${new Date().toISOString()}_`);
            ...
            if ('error' in addCommentResponse || !addCommentResponse.id) {
                ...
                const specificCommentResponse = await bitbucket.getPullRequestComment(TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID, addCommentResponse.id);
            ...
            if ('error' in addCommentResponse || !addCommentResponse.id) {
                ...
                const updateCommentResponse = await bitbucket.updatePullRequestComment(TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID, addCommentResponse.id, updatedCommentText, addCommentResponse.version);
            ...
            if ('error' in createTaskResponse || !createTaskResponse.id) {
                ...
                const specificTaskResponse = await bitbucket.getPullRequestTask(createTaskResponse.id, TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID);
            ...
            if ('error' in createTaskResponse || !createTaskResponse.id) {
                ...
                const updateTaskResponse = await bitbucket.updatePullRequestTask(createTaskResponse.id, `Updated task - ${new Date().toISOString()}`, 'RESOLVED', TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID);
            ...
            console.log('-'.repeat(60));
            const unapproveResponse = await bitbucket.unapprovePullRequest(TEST_PROJECT_KEY, TEST_REPO_SLUG, TEST_PR_ID);
